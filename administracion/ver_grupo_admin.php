<?php
// administracion/ver_grupo_admin.php
...
if (!$id) { header('Location: lista_grupos_admin.php'); exit(); }
...
$stmt->execute([$id]);
$grupo = $stmt->fetch();
if (!$grupo) { header('Location: lista_grupos_admin.php'); exit(); }

// ── Calcular saldo ───────────────────────────────────────────────────────
function calcularSaldoGrupo(PDO $pdo, int $grupo_id): array {
    $stmt = $pdo->prepare("
        SELECT COUNT(id) AS total_acampantes,
...
if ($_POST && ($_POST['accion'] ?? '') === 'agregar_pago') {
    try {
        ...
        $pdo->prepare("
            INSERT INTO pagos_grupo
                (grupo_id, monto, modo_pago, notas, registrado_por, fecha_pago)
            VALUES (?, ?, ?, ?, ?, ?)
        ")->execute([$id, $monto, $modo, $notas_p, $_SESSION['user_id'], $fecha_pago]);
        $pdo->commit();

        registrarLog($pdo, 'pago_grupo',
            "Pago \${$monto} grupo '{$grupo['encargado_nombre']}'",
...
// ── Eliminar pago ────────────────────────────────────────────────────────
if (isset($_GET['accion']) && $_GET['accion'] === 'eliminar_pago'
    && isset($_GET['pago_id'])) {
    try {
...
<div class="row mb-4">
    <div class="col-12 d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">
                        <a href="lista_grupos_admin.php">Grupos</a>
                    </li>
        ...
        <a href="lista_grupos_admin.php" class="btn btn-outline-secondary btn-sm">
...
<div class="row g-4">
    ...
    <div class="col-md-4">
        ...
        <div class="card mb-3">
            <div class="card-body">
                ...
                <div class="small mb-1">
                    <i class="fas fa-phone fa-xs text-muted me-1"></i>
                ...
                <?php endif; ?>
                <?php if ($grupo['notas']): ?>
                <div class="alert alert-light small py-2 mb-0 mt-2">
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h6 class="mb-0">
                    <i class="fas fa-dollar-sign"></i> Estado de Pago
                </h6>
            ...
            <div class="card-body">
                ...
                <div class="progress mb-1" style="height:10px; border-radius:5px;">
                    <div class="progress-bar bg-<?= $color_pago ?>"
                         style="width:<?= $pct ?>%; border-radius:5px;">
                    </div>
                </div>
                ...
                <form method="POST">
                    ...
                    <div class="p-2 rounded bg-light border small mb-3">
                        <div class="d-flex justify-content-between">
                            ...
                            <span id="previewSaldo" class="fw-bold text-danger">
                                $<?= number_format($saldo_info['saldo'], 2) ?>
                            </span>
                        </div>
                        <div id="previewCompleto"
                             class="text-success fw-bold d-none mt-1 text-center small">
                            <i class="fas fa-check-double"></i>
                            ¡Este pago liquida el grupo!
                        </div>
                    </div>
        ...
        <div class="alert alert-success text-center">
            ...
            <strong>¡Grupo liquidado!</strong><br>
            <small>No hay saldo pendiente.</small>
        </div>
        <?php endif; ?>
    ...
    <!-- ── Panel derecho: lista de acampantes (solo lectura) ─────────── -->
    <div class="col-md-8">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <h6 class="mb-0">
                    <i class="fas fa-list"></i> Acampantes
            ...
            <div class="card-body text-center text-muted py-4">
                ...
                <p>Sin acampantes en este grupo.</p>
            </div>
            <?php else: ?>
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    ...
                    foreach ($acampantes as $i => $ac):
                        $total_costo += (float)$ac['costo_total'];
                    ?>
                    <tr class="<?= $ac['llego'] ? 'table-success' : '' ?>">
                        ...
                        <td class="text-center">
                            <?php if ($ac['documentos_revisados']): ?>
                                <span class="badge bg-success"
                                      title="<?= date('d/m/Y H:i', strtotime($ac['documentos_revisados_at'])) ?>">
                                    <i class="fas fa-check-double fa-xs"></i> OK
                                </span>
                            <?php else: ?>
                        ...
                        <td class="text-center">
                            <?php if ($ac['llego']): ?>
                                <span class="badge bg-primary"
                                      title="<?= $ac['fecha_llegada']
                                          ? date('d/m/Y H:i', strtotime($ac['fecha_llegada']))
                                          : '' ?>">
                                    <i class="fas fa-sign-in-alt fa-xs"></i> ✓
                                </span>
                            <?php else: ?>
...
<script>
const saldoActual = <?= json_encode((float)$saldo_info['saldo']) ?>;
const inputMonto  = document.getElementById('inputMontoGrupo');
const preview     = document.getElementById('previewSaldo');
const previewOk   = document.getElementById('previewCompleto');

if (inputMonto) {
    inputMonto.addEventListener('input', () => {
        const abono    = parseFloat(inputMonto.value) || 0;
        const restante = Math.max(0, saldoActual - abono);
        preview.textContent = '$' + restante.toLocaleString('es-MX', {
            minimumFractionDigits: 2, maximumFractionDigits: 2
        });
        if (abono >= saldoActual && abono > 0) {
            preview.className  = 'fw-bold text-success';
            previewOk.classList.remove('d-none');
        } else {
            preview.className  = 'fw-bold text-danger';
            previewOk.classList.add('d-none');
        }
    });
}
</script>
